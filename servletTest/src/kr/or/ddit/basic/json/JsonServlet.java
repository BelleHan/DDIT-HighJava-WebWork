...
public class JsonServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		switch(choice) {
			case "str" : String str = "안녕하세요";
				jsonData = gson.toJson(str);
				break;
			case "array" : int[] arr = {100, 200, 300, 400};
				jsonData = gson.toJson(arr);
				break;
			case "obj" : SampleVO samVo = new SampleVO(1, "홍길동");
				jsonData = gson.toJson(samVo);
				break;
			case "list" : List<SampleVO> samList = new ArrayList<>();
				...
				break;
			case "map" : Map<String, String> map = new HashMap<>();
				map.put("name", "이순신");
		...
		response.setContentType("application/json; charset=utf-8"); // 제이슨의 응답 데이터라는 뜻
		
		PrintWriter out = response.getWriter();
		out.write(jsonData);
		response.flushBuffer();  // 응답데이터를 모두 출력한다.
